...
A self-contained, end-to-end OpenTelemetry proof of concept: two instrumented
demo services plus a load generator feed **traces, metrics and logs** through
an **OpenTelemetry Collector** into the Grafana stack — with **Prometheus**
as the metrics backend (instead of Mimir).
...
|----------------|----------------------------------------------|------------------------------------------------|-----------|
| checkout       | built from `apps/checkout`                   | Demo API; entry point, calls payments          | 8080      |
| payments       | built from `apps/payments`                   | Demo downstream; simulates declines/errors     | —         |
| loadgen        | built from `apps/loadgen`                    | Continuous realistic traffic (incl. bad calls) | —         |
...
- **Dashboard**: *OpenTelemetry POC — Service Overview* (folder "OpenTelemetry POC"):
  request rate, error rate, latency percentiles, business metrics, live logs.
...
Generate a request yourself:

```bash
curl -s -X POST http://localhost:8080/checkout \
  -H 'content-type: application/json' \
  -d '{"customer_id": "cust-001", "items": {"keyboard": 1, "monitor": 2}}'
```
...
- Custom app metrics (`orders_created_total`, `payments_processed_total`,
  `order_value_USD_*`) in Prometheus — OTLP metrics path works.
- `{service_name="checkout"}` in Loki returns structured logs carrying
  `trace_id` — OTLP logs path + log/trace correlation works.
- Tempo trace search shows `checkout → payments` traces, including error
  traces for declined (402) and failed (500) payments.
...
- **Auto + manual instrumentation**: the Python apps use
  ...
  plus manual custom spans, counters and histograms — demonstrating both
  approaches.
...
```
apps/                 # demo services (FastAPI) + load generator
